...
const windowsUserAgent = "Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/122.0.0.0 Safari/537.36";
...
function formatDeltaMessage(m) {
    ...
    // OLD METHOD: Extract from delta.data.prng if available
    ...
    if (md?.data?.data) {
        try {
            ...
            console.log('[MENTION PARSE ERROR] messageMetadata.data.data:', e.message);
        }
    }
...
function parseAndCheckLogin(ctx, http, retryCount) {
    ...
    return function(data) {
        function any() {
            ...
            if (data.statusCode !== 200) throw new Error("Status code: " + data.statusCode);
            let res = null;
            try { res = JSON.parse(makeParsable(data.body)); } catch (e) { throw e; }
            if (res.redirect && data.request.method === "GET") return http.get(res.redirect, ctx.jar).then(parseAndCheckLogin(ctx, http));
